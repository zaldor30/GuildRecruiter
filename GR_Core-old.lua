...
function core:LoadTables()
    ...
    ns.BlackList = blSuccess and tblBL or {}

    local asSuccess, tblAS = ns.code:decompressData(ns.guild.antiSpamList)
    ns.AntiSpamListList = asSuccess and tblAS or {}
...
function core:PerformRecordMaintenance()
    ...
    removeOldRecords(ns.AntiSpamListList, ns.gmSettings.antiSpamDays)
end
...
function core:StartMiniMapIcon()
    ...
    local iconData = LibStub("LibDataBroker-1.1"):NewDataObject("GR_Icon", {
        ...
        OnTooltipShow = function()
            ...
            local count = 0
            for _ in pairs(ns.AntiSpamListList) do count = count + 1 end
            local antiSpam = ' |cFFFF0000'..count..'|r'

            count = 0
            for _ in pairs(ns.BlackList) do count = count + 1 end
