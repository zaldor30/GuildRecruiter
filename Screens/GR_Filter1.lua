...
function filter:FilterTable()
    ...
    -- Filter Record Table
    function tblFunc:fNew(recordID, name, who, race, class)
        local newRecord = {
            recordID = recordID or nil,
            name = name or nil,
            whoCommand = who or nil,
            RACE = race or {},
            CLASS = class or {},
        }
    ...
    function tblFunc:fGetList()
        ...
        for _, v in pairs(tblFilters) do
            tblList[v.name] = v.name
        end

        return tblList and tblList or {}
    end
    ...
    function tblFunc:BuildFilterTopUI()
        ...
        combobox:SetCallback('OnValueChanged', function(_, _, val)
            ...
            for _, v in pairs(nsTblFilter) do
                if v.name == val then
                    filter.tblFilter = filter.filterData:fNew(val, v.name, v.whoCommand, v.RACE, v.CLASS)
                    for k,r in pairs(v.CLASS) do print(k,r) end
                    break
                end
            end
            --local tbl = filter.tblFilter[val]
            --filter.tblFilter = filter.filterData:fNew(val, tbl.name, tbl.whoCommand, tbl.RACE, tbl.CLASS)
        ...
        btnSave:SetCallback('OnClick', function()
            ...
            local tblFilters = ns.dbGlobal.filterList and ns.dbGlobal.filterList or {}
            filter.tblFilter.recordID = nil
            tinsert(tblFilters, filter.tblFilter)
            tblBase.statusText:SetText(L['Filter Saved']..' '..filter.tblFilter.name)

            tblFrame.inline.filterCombo:SetList(filter.filterData:fGetList())
            tblFrame.inline.filterCombo:SetValue(filter.tblFilter.name)
        ...
        whoEditbox:SetCallback('OnEnterPressed', function(_, _, text)
            filter.filterData:fSave(nil, text)
        end)
