...
function ds:Init()
    ...
    -- Debug output
    print("Initializing dataset - Game version:")
    print("Retail:", ns.retail)
    print("Classic:", ns.classic)
    print("Cata:", ns.cata)

    -- Initialize pSettings if it doesn't exist
    ns.pSettings = ns.pSettings or {}
    ...
    if not ns.pSettings.classFilters then
        ...
        for classFile, _ in pairs(self:GetClassList()) do
    ...
    if not ns.pSettings.raceFilters then
        ...
        for raceName, _ in pairs(self:GetRaceList()) do
...
function ds:races_retail() -- Race data
    ...
    -- Known faction associations for retail races
    local raceFactions = {
        [1] = "Alliance",    -- Human
        [2] = "Horde",       -- Orc
        [3] = "Alliance",    -- Dwarf
    ...
    -- Loop through all possible race IDs
    for raceID = 1, 100 do
        local raceInfo = C_CreatureInfo.GetRaceInfo(raceID)
        if raceInfo and raceFactions[raceInfo.raceID] then
            -- Only add races for the player's faction
    ...
    for classID = 1, GetNumClasses() do
        ...
        if className then
            tbl[classFile] = {
                id = classID,
                name = className,
                icon = icons[classFile] or 134400, -- Default icon if none specified
...
-- Helper function to get appropriate class list based on game version
function ds:GetClassList()
    if ns.retail then
        return self:classes_retail()
    elseif ns.cata then
        return self:classes_cata()
    else
        return self:classes_classic()
    end
end

-- Helper function to get appropriate race list based on game version
function ds:GetRaceList()
    if ns.retail then
        return self:races_retail()
    elseif ns.cata then
        return self:races_cata()
    else
        return self:races_classic()
    end
end
